export function changeBgIfQueryIsUkr(query) {
    const bgContainer = document.querySelector('.container--header');
    ...
    if (!query.toLowerCase().includes('ukrain')) {
        ...
        if(screen.width < 768) {
            ...
            // Мобильный не совсем помещается, а в десктопном приходится добавлять  этот градиент странный для закрывания куска надпии :/
             const image = 'linear-gradient(rgb(229 175 25/90%) 2%, rgb(229 175 25 / 75%) 12%, transparent 70%), url(https://www.shutterstock.com/image-photo/sunflower-blue-heart-shaped-center-600w-2018098397.jpg)';
            bgContainer.style.backgroundImage =image;     
            ...
            const image = 'url(https://www.shutterstock.com/image-photo/sunflower-blue-heart-shaped-center-600w-2018098397.jpg)';
